...
export const evaluateMemoQualityGate = (params: {
  ...
  const hasMemoRequiredFailures = params.grade.requiredFailures.length > 0;
  const modelQualityScore = hasMemoRequiredFailures ? 0 : params.grade.score;
  const checks: QualityGateCheck[] = [
    ...
    makeCheck({
      name: "model_quality",
      detail: `memo_grade_score=${params.grade.score.toFixed(2)} memo_grade_min=${params.grade.minScore.toFixed(2)} memo_grade_passed=${params.grade.passed ? 1 : 0} memo_grade_required_failures=${hasMemoRequiredFailures ? params.grade.requiredFailures.join(",") : "none"}`,
      weight: 0.14,
  ...
  return finalizeChecks({
    ...
    metrics: {
      ...
      memo_grade_required_failures: params.grade.requiredFailures.length,
    },
  });
};
